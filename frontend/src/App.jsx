...
function App() {
  ...
  const checkHealth = async () => {
    try {
      ...
      setConnectionError(response.ok ? '' : `Health check failed (${response.status})`)
    } catch {
      setIsConnected(false)
      setConnectionError(`Cannot reach backend at ${API_BASE_URL}`)
    }
  }
    ...
    <main className="page-shell">
      <section className="chat-card">
        ...
        <div className="chat-history" ref={historyRef}>
          {connectionError && <p className="connection-warning">{connectionError}</p>}
          {messages.length === 0 && (
        ...
        <footer className="chat-input-area">
          <small className="api-hint">API: {API_BASE_URL}</small>
          <input
            type="text"
            value={input}
